    ...
    gsapTimeline
      ...
      .to(
        'main',
        {
          ...
          border: `2px solid ${getComputedStyle(document.documentElement).getPropertyValue('--sectionBorder').trim()}`,
          scale: 0.9,
          pointerEvents: 'none',
          left: '-40vw',
        },
  ...
  return (
    <nav id="menu" ref={menuRef} className={styles.menu}>
      <div className={clsx(styles.menuWrapper, 'layout-block-inner')}>
        ...
        >
          ...
          >
            <Link
              ...
              scroll={false}
              href="mailto:gyanranjanpriyam@gmail.com"
            >
        ...
        <div
          ref={(el) => {
            menuLinksItemsRef.current[menuLinks.length + 3] = el;
          }}
          className={styles.menuList}
        >
          {footerLinks.map((link, index) => (
            <div
              ref={(el) => {
                menuLinksItemsRef.current[menuLinks.length + index + 3] = el;
              }}
              key={link.title}
              className={styles.menuListItem}
            >
